...
export default function DashboardProductsPage() {
  ...
  const submitProduct = (event: FormEvent) => {
    ...
    const payload = { ...productForm, categoryId, price: Number(productForm.price), preparationTime: Number(productForm.preparationTime) }
    if (editingProductId) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-slate-500 dark:text-slate-400">أي تغيير هنا يظهر مباشرة في صفحة القائمة والصفحة الرئيسية.</p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{editingCategoryId ? 'تعديل قسم' : 'إضافة قسم'}</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{editingProductId ? 'تعديل منتج' : 'إضافة منتج'}</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={submitProduct} className="grid gap-4 md:grid-cols-2">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <Input id="image" value={productForm.image} onChange={(event) => setProductForm({ ...productForm, image: event.target.value })} />
                  </div>
                </div>
              ...
              {products.length === 0 ? (
                ...
                products.map((product) => (
                  <div key={product.id} className="grid gap-4 rounded-md border border-slate-200 p-4 dark:border-slate-800 md:grid-cols-[80px_1fr_auto]">
                    <div className="flex h-20 w-20 items-center justify-center rounded-md bg-red-50 text-4xl dark:bg-red-950">{product.image}</div>
                    <div>
